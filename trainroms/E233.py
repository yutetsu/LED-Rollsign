...
class E233():
    ...
    def DestOnlyMode(self):
        ...
        if self.args['Dest'] is not None : 
            DestFrame =  Image.open(f"trainroms/data/{self.data['Name']}/DestOnly/{self.args['Dest']}.png")
    ...
    def NextMode(self):
        ...
        if self.args['Type'] is not None : 
            ...
            img.paste(TypeFrame, (0, 0))
        if self.args['Dest'] is not None : 
            DestFrame =  Image.open(f"trainroms/data/{self.data['Name']}/DestNext/{self.args['Dest']}_JP.png")
            img.paste(DestFrame, (48, 0))
        if self.args['Next'] is not None : 
            DestFrame =  Image.open(f"trainroms/data/{self.data['Name']}/Next/{self.args['Next']}_JP.png")
            img.paste(DestFrame, (48, 16))
        Framebuffer.append(img.convert("P", palette=Image.ADAPTIVE, colors=self.data['Colors']))
        img = Image.new("RGB", (self.data['Width'], self.data['Height']), (0, 0, 0))
        if self.args['Type'] is not None : 
            ...
            img.paste(TypeFrame, (0, 0))
        if self.args['Dest'] is not None : 
            DestFrame =  Image.open(f"trainroms/data/{self.data['Name']}/DestNext/{self.args['Dest']}_EN.png")
            img.paste(DestFrame, (48, 0))
        if self.args['Next'] is not None : 
            DestFrame =  Image.open(f"trainroms/data/{self.data['Name']}/Next/{self.args['Next']}_EN.png")
            img.paste(DestFrame, (48, 16))
        Framebuffer.append(img.convert("P", palette=Image.ADAPTIVE, colors=self.data['Colors']))
        MakeGif()
