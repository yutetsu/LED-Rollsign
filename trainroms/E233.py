...
class E233():
    ...
    def TypeOnlyMode(self):
        img = Image.new("RGB", (self.data['Width'], self.data['Height']), (0, 0, 0))
        if self.args['Type'] is not None : 
            TypeFrame =  Image.open(f"trainroms/data/{self.data['Name']}/TypeOnly/{self.args['Type']}.png")
    ...
    def LineOnlyMode(self):
        ...
        if self.args['Line'] is not None : 
            LineFrame =  Image.open(f"trainroms/data/{self.data['Name']}/LineOnly/{self.args['Line']}.png")
